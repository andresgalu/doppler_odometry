...
public:
    ...
    //friend std::ostream& operator<<(std::ostream& out, const Point& p) { return out << float(p.x) << ", " << float(p.y); }
...
public:
    ...
    //friend std::ostream& operator<<(std::ostream& out, const Line& l) { return out << float(l.a) << ", " << float(l.b) << ", " << float(l.c); }
